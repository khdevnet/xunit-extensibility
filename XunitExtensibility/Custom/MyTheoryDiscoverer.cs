...
namespace CustomTestClassExecution.Custom
{
    public class MyTheoryDiscoverer : IXunitTestCaseDiscoverer
    {
        ...
        protected virtual IXunitTestCase CreateTestCaseForDataRow(ITestFrameworkDiscoveryOptions discoveryOptions, ITestMethod testMethod, IAttributeInfo theoryAttribute, object[] dataRow)
            => new MyTestCase(DiagnosticMessageSink, discoveryOptions.MethodDisplayOrDefault(), discoveryOptions.MethodDisplayOptionsOrDefault(), testMethod, dataRow);
...
#pragma warning restore CS0618
        ...
        protected virtual IXunitTestCase CreateTestCaseForTheory(ITestFrameworkDiscoveryOptions discoveryOptions, ITestMethod testMethod, IAttributeInfo theoryAttribute)
            => new MyTheoryTestCase(DiagnosticMessageSink, discoveryOptions.MethodDisplayOrDefault(), discoveryOptions.MethodDisplayOptionsOrDefault(), testMethod);
...
#pragma warning restore CS0618
        ...
        public virtual IEnumerable<IXunitTestCase> Discover(ITestFrameworkDiscoveryOptions discoveryOptions, ITestMethod testMethod, IAttributeInfo theoryAttribute)
        {
            ...
            if (discoveryOptions.PreEnumerateTheoriesOrDefault())
            {
                try
                {
                    var dataAttributes = ReflectionAbstractionExtensions.GetCustomAttributes(testMethod.Method, typeof(DataAttribute));
                    ...
                    foreach (var dataAttribute in dataAttributes)
                    {
                        ...
                        foreach (var dataRow in data)
                        {
                            ...
                            if (!Xunit.Sdk1.SerializationHelper.IsSerializable(dataRow))
                            {
